...
export class ImageUriResolver {
  ...
  static needsSync(): boolean {
    return IMAGE_URI_PARTS[2].size === 0;
  }
  ...
  static getImageUri(
    ...
    switch (extension) {
      ...
      case "jxl": {
        if (
          image[
            `has${extension[0].toUpperCase()}${extension.slice(1)}` as keyof HitomiFile
          ]
        ) {
          break;
        }
        throw new HitomiError(HitomiErrorCode.INVALID_VALUE, "extension");
      }
      case "gif": {
        // GIF is the original format — no availability flag needed
        break;
      }
      default: {
        throw new HitomiError(HitomiErrorCode.INVALID_VALUE, "extension");
    ...
    return `https://${subdomain}.${IMAGE_DOMAIN}/${path}.${extension}`;
  }
}
