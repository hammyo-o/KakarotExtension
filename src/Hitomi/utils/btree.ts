...
async function getGalleriesIndexVersion(
  ...
  const now = Date.now();
  if (cachedVersion && now - cachedVersionTs < VERSION_CACHE_TTL_MS) {
...
export type BTreeFetcher = (url: string, headers?: Record<string, string>) => Promise<ArrayBuffer>;
...
async function bTreeSearch(
  ...
  for (let depth = 0; depth < 20; depth++) {
    ...
    logBTree(
      `depth=${depth} nKeys=${node.keys.length} isLeaf=${isLeaf(node)}`,
    );
...
function parseGalleryIdsFromData(buffer: ArrayBuffer): number[] {
  ...
  if (buffer.byteLength !== needed) {
    console.warn(
      `[Hitomi BTree] Data length mismatch: got ${buffer.byteLength}, expected ${needed}`,
    );
    return [];
  }
...
export async function searchGalleryIdsByTerm(
  ...
  if (!dataRange) {
    logBTree(`Term "${normalized}" not found in index`);
    return [];
  }
